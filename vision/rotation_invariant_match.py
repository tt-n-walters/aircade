...
def get_matched_coordinates(temp_img, map_img):
    ...
    # result image path
    cv2.imwrite(os.path.join("vision", 'result.png'), img3)
...
if __name__ == "__main__":
    ...
    img_template = os.path.join(save_dir, "")
    img_map = os.path.join(save_dir, "")

    # read images
    temp_img_gray = cv2.imread(img_template, 0)
    map_img_gray = cv2.imread(img_map, 0)
